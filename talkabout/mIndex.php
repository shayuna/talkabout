<?php
    session_start();
    $s=$_GET["s"];
    $sbjAreaHdrTxt="Click on a subject, start tolking";
...
<script language="javascript" type="text/javascript">
    ...
    $(document).ready(function(){
       ...
       $(window).resize(function(){
        ...
        $("#isRadar").bind("change",chngRadarStatus); 
//		$("#isRadarArea").css("display","none");
        $("#isRadarArea").hide();
  /*
...
*/
    ...
    function activateMsg(isAddSubject)
    {
    ...
    function adjustScrn()
    {
        ...
        $("#isDitchMyDlgBox").css("height","200px");
        $("#isDitchYourDlgBox").css("height","200px");
        var fromBtm=myHeight/2-100;
        $("#isDitchMyDlgBox").css("bottom",fromBtm);
        $("#isDitchYourDlgBox").css("bottom",fromBtm);
    }
    function onLoad()
    {
        ...
        $.getJSON("../updateSubjectsList.php",function(data) {
           ...
           $.each(data,function(k,v){
                ...
                if (isMine=="1")
                {
                      ...
                      imgClass="tolkSubjectOptionItem showMe";
                }
                else
                {
...
*/  
            ...
            //                   var tolkBoxTxt=$("#newSubjectTolkBox").html()+msgAr[0];
                            ...
                            else if (!isNaN(tmpAr1[0]))
                            {
                               ...
                               if (msgTxt.indexOf(sbjStartTolkMsg)>-1)
                               {
                                   var msgToUser="there is some action going on in 'my subject' room";
                                   openUsrMsg(msgToUser);
                               }
                            }
                        }
                   }
            ...
            //                   var tolkBoxTxt=$("#fromListSubjectTolkBox").html()+msgAr[1];
                            ...
                            else
                            {
                               ...
                               if (msgTxt.indexOf(sbjStartTolkMsg)>-1)
                               {
                                   var msgToUser="there is some action going on in 'your subject' room";
                                   openUsrMsg(msgToUser);
                               }
                            }
                        }
...
//            $("#newSubjectPostInput").attr("value","");
  ...
  //          $("#fromListSubjectPostInput").attr("value","");
    ...
    function ditchResult(toContinue,fromBoxNo)
    {
           toggleDitchBox(0,fromBoxNo);
...
//              openUsrMsg("oopps! something happened. wait a second or two before trying to add your new subject again");
        ...
        {
            setAddNewSubjectLock(true);
            $.ajax({url: "../addNewSubject.php?newSubjectInput="+newSbj,
                    error: function(data) {
                        setAddNewSubjectLock(false);},
                    success: function(data) {
                        dataAr=data.split("*&*");
  ...
  /*
    ...
    */
    ...
    function toggleDitchBox(toShow,fromBoxNo)
    {
        if (toShow)
        {
           var msgToShow="";
           if (fromBoxNo==1)msgToShow="partner in my subject conversation wants to leave the conversation.you can decide.";
           else msgToShow="partner in your subject conversation wants to leave the conversation.you can decide.";
           openUsrMsg(msgToShow);
        
            if (fromBoxNo==1)$("#isDitchMyDlgBox").css("display","");
            else $("#isDitchYourDlgBox").css("display","");
            $("#freeWillieBtn").focus();
        }
        else
        {
            if (fromBoxNo==1)$("#isDitchMyDlgBox").css("display","none");
            else $("#isDitchYourDlgBox").css("display","none");
            $("#freeWillieBtn").focus();
        }
    ...
    function toggleOptions(obj,opt) {
        ...
        // for the mean time, in the mobile version, we don't want to hide the owner subject's options
                return true;
...
//                    $("#curtain").removeClass("curtainDown").addClass("curtainUp");
    ...
    {
        msgInNewTolkBox="start tolking about your subject";
        msgInFromListTolkBox="start tolking about subject from list";
...
/*
           if (isNewBox)document.location.href="#newSubjectPostInput";
           else document.location.href="#fromListSubjectPostInput";
*/
    }
    ...
    function chngRadarStatus()
    {
        if ($("#isRadar").attr("value")=="0") $("#isRadarArea").hide("slow");
        else  $("#isRadarArea").show("slow");
    }
</script>
</head>
<body onload="onLoad()">                                            
<div data-role="page" id="home">
    ...
    <div data-role="content">
        <ul data-role="listview" data-inset="true">
            <li><a href="#subjectsListPage" data-transition="flip">Subjects List</a></li>
            <li><a href="#mySubjectPage" data-transition="flip">My Subject</a></li>
            <li><a href="#yourSubjectPage" data-transition="flip">Your Subject</a></li>
<!--            <li><a href="#setMe" data-transition="flip">Set me Straight</a></li> -->
        </ul>
    </div>
...
<div data-role="page" id="subjectsListPage">
    ...
    <div data-role="content">
        ...
        <div id="manageSubjectsArea">
            ...
            <div id="subjectsListCaption" class="subjectsArea">
            ...
            <div class="subjectsArea">
                <div data-inset="true">
                    <input id="newSubjectInput" name="newSubjectInput"  
                        placeholder="<?=$sbjInputPlaceHolder?>"
                        title="<?=$sbjInputTtl?>" onkeyDown="return isAddNewSubject(event)"/>
                    <script language="javascript">
                        if (!("placeholder" in document.createElement("input"))) document.getElementById("newSubjectInput").focus();
                    </script>
                    <button id="newSubjectAdd" data-role="button" 
                        onkeyDown="return isAddNewSubject(event)" onclick="addNewSubject()"><?=$sbjAddBtnTxt?></button>
                </div>
            </div>
        </div>
...
<div data-role="page" id="mySubjectPage">
    <div data-role="header" data-theme="e" data-position="fixed">
        <h1>Tolkabout</h1>
    </div>
    <div data-role="content">
         ...
         <div class="tolkArea" id="myTolkArea">
            ...
            <div class="tolkBackBox" >
                <div>
                     <textarea class="postInput" id="newSubjectPostInput" style="height:62px;" onfocus="chkTolkBoxOnEnter(this,1)" onblur="chkTolkBoxOnExit(this,1)" onkeydown="updateTypingStatus(this)"></textarea>
                 </div>
                 <div>
        ...
        <div id="isDitchMyDlgBox" class="msgToUser msgToUser_Small">
            <div>
                <span id="ditchMsgTxt"><?=$sbjDitchMsg?>
                </span>
            </div>

            <div class="ditchBoxBtnsLine">
                <input type="button" id="freeWillieBtn" value="<?=$ditchBtnFreeTxt?>" onclick="ditchResult(1,1)" /> 
                <input type="button" id="ignoreBtn" value="<?=$ditchBtnIgnoreTxt?>" onclick="ditchResult(0,1)" />
            </div>
        </div>
...
<div data-role="page" id="yourSubjectPage">
    <div data-role="header" data-theme="e" data-position="fixed">
        <h1>Tolkabout</h1>
    </div>
    <div data-role="content">
        ...
        <div class="tolkArea" id="listTolkArea">
            ...
            <div class="tolkBackBox" >
                <div>
                     <textarea class="postInput" id="fromListSubjectPostInput" style="height:62px;" onfocus="chkTolkBoxOnEnter(this,0)" onblur="chkTolkBoxOnExit(this,0)" onkeydown="updateTypingStatus(this)"></textarea>
                 </div>
                 <div>
        ...
        <div id="isDitchYourDlgBox" class="msgToUser msgToUser_Small">
            <div>
                <span id="ditchMsgTxt"><?=$sbjDitchMsg?>
                </span>
            </div>
            ...
            <div class="ditchBoxBtnsLine">
                <input type="button" id="freeWillieBtn" value="<?=$ditchBtnFreeTxt?>" onclick="ditchResult(1,2)" />
                <input type="button" id="ignoreBtn" value="<?=$ditchBtnIgnoreTxt?>" onclick="ditchResult(0,2)" />
            </div>
        </div>
    </div>
    <div data-role="footer" data-theme="e">
...
<div data-role="page" id="setMe" data-theme="a">
    <div data-role="header" data-theme="b" data-position="fixed">
        <h1>Tolkabout</h1>
    </div>
    <div data-role="content">
        <div data-role="fieldcontain">
            <select id="isRadar" data-theme="b">
                <option value="0">disable radar</option>
                <option value="1">enable radar</option>
            </select>
        </div>
        <div data-role="fieldcontain" id="isRadarArea">
            <label for="rd" >impact radius (in meters):</label>
            <input type="range" id="rd" value="0" min="0" max="1000" data-theme="b" />
        </div>
    </div>
